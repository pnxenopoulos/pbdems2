...
macro_rules! impl_try_from_ref_integer {
    ($($ty:ty),+ $(,)?) => {
        $(
            impl TryFrom<&FieldValue> for $ty {
                type Error = FieldValueConversionError;
                ...
                fn try_from(value: &FieldValue) -> std::result::Result<Self, Self::Error> {
                    match value {
                        FieldValue::I32(value) => {
                            <$ty>::try_from(*value).map_err(|_| FieldValueConversionError)
                        }
...
impl_try_from_ref_integer!(i8, i16, i32, i64, u8, u16, u32, u64);
...
impl TryFrom<&FieldValue> for [f32; 2] {
    ...
    fn try_from(value: &FieldValue) -> std::result::Result<Self, Self::Error> {
        match value {
            FieldValue::Vector2(value) => Ok(*value),
            _ => Err(FieldValueConversionError),
        }
    }
}
...
impl TryFrom<&FieldValue> for [f32; 4] {
    ...
    fn try_from(value: &FieldValue) -> std::result::Result<Self, Self::Error> {
        match value {
            FieldValue::Vector4(value) => Ok(*value),
            _ => Err(FieldValueConversionError),
        }
    }
}
