...
impl<'a> BitReader<'a> {
    ...
    /// Peek at up to 64 bits without advancing the cursor.
    #[inline]
    ...
    fn peek_bits_unchecked(&self, n: usize) -> u64 {
        ...
        let word = if remaining_bytes >= 8 {
            ...
            self.data[byte_pos..]
                .iter()
                .enumerate()
                .fold(0u64, |word, (i, &byte)| word | (byte as u64) << (i * 8))
        };
    ...
    pub fn read_bool(&mut self) -> Result<bool> {
        ...
        let value = self.data[self.position / 8] & (1 << (self.position % 8)) != 0;
        self.position += 1;
        Ok(value)
    }
    ...
    pub fn read_u8(&mut self) -> Result<u8> {
        ...
        let value = if bit_offset == 0 {
            ...
            let window = self.data[byte_pos] as u16 | (self.data[byte_pos + 1] as u16) << 8;
            (window >> bit_offset) as u8
        };
        self.position += 8;
        Ok(value)
    }

    /// Read 16 bits as a little-endian integer.
    #[inline]
    ...
    /// Read 64 bits as a little-endian integer.
    #[inline]
    ...
    /// Read a raw null-terminated string while enforcing a byte limit.
    pub fn read_string_raw_limited(
        &mut self,
        buf: &mut Vec<u8>,
        max_bytes: usize,
    ) -> Result<usize> {
        ...
        loop {
            ...
            if written == max_bytes {
                return Err(Error::LimitExceeded {
                    resource: "decoded field string",
                    limit: max_bytes,
                    actual: written.saturating_add(1),
                });
            }
            buf.push(b);
        }
...
mod tests {
    ...
    fn test_read_u8_unaligned() {
        let data = [0b1011_0100, 0b1100_1010, 0b0110_0001];
        for offset in 1..8 {
            let mut br = BitReader::new(&data);
            br.skip_bits(offset).unwrap();
            let got = br.read_u8().unwrap();
            let window = u32::from_le_bytes([data[0], data[1], data[2], 0]);
            assert_eq!(got, ((window >> offset) & 0xff) as u8);
        }
    }

    #[test]
    fn test_read_bool_overflow() {
        let data = [0xff];
        let mut br = BitReader::new(&data);
        for _ in 0..8 {
